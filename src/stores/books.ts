...
export const useBooksStore = defineStore("books", () => {
  ...
  function update(book: Book) {
    const index = books.value.findIndex((b) => b.id.uuid === book.id.uuid);
...
function getBooksFromLocalStorage(): Book[] | undefined {
  const books = getFromLocalStorage<Book[]>(BOOKS_LOCAL_STORAGE_KEY);
  return books?.map((book) => ({
    ...book,
    publishedAt: new Date(book.publishedAt),
    coverUrl: new URL(book.coverUrl),
  }));
}
